...
ParSimulatedClassificationCrossDatasets <- function(data,
                                    parameters, 
                                    n_sample = 5, 
                                    sample_incr = 20, 
                                    n_protein = 5,
    ...
    nreps <- sample_incr * n_sample * ngroup
    ...
    results <- foreach(i=1:iter) %:% foreach(n=protein_num) %:% foreach(m=train_size, .packages="caret") %dopar% {
      source("scripts/ParSampleSizeCalculationClassification.R", local=TRUE)
      message("  Training Iteration ",i,", ",n," Proteins, ",m," Biological Replicates")
      prot_index <- which(protein_num==n)
      selectedPros<-order(promean,decreasing = TRUE)[1:protein_num[prot_index]]
      mu_2 <- mu[selectedPros,]
      sigma_2 <-sigma[selectedPros,]
      ##Simulate training data 
      train_index <- which(train_size==m)
      train <- .sampleSimulation(m, mu_2, sigma_2) 
      ...
      message("Completed  Iteration ",i,", ",n," Proteins, ",m," Biological Replicates")
      run <- list(acc, model)
    ...
    meanPA <- matrix(rep.int(0, times=n_protein*n_sample))
    dim(meanPA) = c(n_protein,n_sample)
    rownames(meanPA) <- protein_num
    colnames(meanPA) <- train_size
    
    for (n in 1:n_protein) {
      for (m in 1:n_sample) {
        for (i in 1:iter) {
          meanPA[n,m] <- mean(results[[i]][[n]][[m]][[1]])
        }
      }
    }
    ...
    paramlog <- list("n_sample"=n_sample, "sample_incr" = sample_incr, "protein_desc" = protein_desc, "iter" = iter, "classifier" = classifier, "protein_num" = protein_num, "train_size" = train_size)
    
    return(list(results = results, meanPA = meanPA, paramlog = paramlog))
    # return(results)
}
